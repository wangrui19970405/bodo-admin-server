...
func (b *BaseApi) Captcha(c *gin.Context) {
	// 字符,公式,验证码配置
	// 生成默认数字的driver
	driver := base64Captcha.NewDriverDigit(global.BODO_CONFIG.Captcha.ImgHeight, global.BODO_CONFIG.Captcha.ImgWidth, global.BODO_CONFIG.Captcha.KeyLong, 0.7, 80)
	// cp := base64Captcha.NewCaptcha(driver, store.UseWithCtx(c))   // v8下使用redis
	cp := base64Captcha.NewCaptcha(driver, store)
	id, b64s, err := cp.Generate()
	if err != nil {
		global.BODO_LOG.Error("验证码获取失败!", zap.Error(err))
	...
	response.OkWithDetailed(systemRes.SysCaptchaResponse{
		CaptchaId:     id,
		PicPath:       b64s,
		CaptchaLength: global.BODO_CONFIG.Captcha.KeyLong,
	}, "验证码获取成功", c)
}
